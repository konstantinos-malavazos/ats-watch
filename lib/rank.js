...
// DeepSeek. Everything is env-configurable because the exact base URL could
// not be confirmed from the authoring environment (egress was blocked), so if
// the default is wrong it is a one-line env change rather than a code change:
//
  ...
  try {
    const res = await fetchImpl(url, {
      ...
      body: JSON.stringify({
        ...
        temperature: 0,
        max_tokens: Math.min(8192, 256 + jobs.length * 160),
      }),
    ...
    if (typeof text !== 'string' || !text.trim()) {
      log?.error('ranker returned no message content');
      return null;
    }

    const ranks = parseRankingResponse(text, jobs);
    if (!ranks) {
      log?.error(`ranker response was not the JSON we asked for: ${trimForLog(text)}`);
      return null;
    }
