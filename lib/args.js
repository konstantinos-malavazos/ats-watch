...
Options:
  ...
  --min-score <n>     Only print roles the ranker scored at least <n> (0-10).
                       Jobs the ranker did not score, and runs with no
                       ranking at all, are never dropped by this.
  --quiet             Suppress info-level stderr chatter.
  --no-rank           Skip ranking and print the unranked list.
  --no-title-filter   Send every title to the ranker, including the sales and
                       back-office roles the title filter normally drops.
  --format <kind>     Output format: text (default) or discord. text is the
                       plain, budgeted digest; discord is markdown for the
                       delivery path and is not truncated.
 ...
 *   statePath: string|null, limit: number|null, minScore: number|null,
 *   quiet: boolean, noRank: boolean, titleFilter: boolean,
 *   format: 'text'|'discord', help: boolean,
 * }}
 * @throws {Error} on an unknown option, a missing value, or an invalid
 *   --since / --limit / --min-score.
 */
export function parseCliArgs(argv = process.argv.slice(2)) {
  ...
  try {
    ({ values } = parseArgs({
      ...
      options: {
        ...
        'no-title-filter': { type: 'boolean', default: false },
        format: { type: 'string' },
  ...
  return {
    ...
    titleFilter: !values['no-title-filter'],
    format: parseFormat(values.format),
    help: values.help,
  };
}

const FORMATS = ['text', 'discord'];

/** Validate --format, defaulting to the plain-text digest. */
function parseFormat(raw) {
  if (raw === undefined) return 'text';
  const v = String(raw).trim().toLowerCase();
  if (!FORMATS.includes(v)) {
    throw new Error(`Invalid --format "${raw}": expected one of ${FORMATS.join(', ')}.`);
  }
  return v;
}
...
/** Parse --min-score into an integer 0-10, or null when absent. */
function parseMinScore(raw) {
  if (raw === undefined) return null;
  const t = raw.trim();
  if (!/^\d+$/.test(t) || Number(t) > 10) {
    throw new Error(`Invalid --min-score "${raw}": expected an integer between 0 and 10.`);
  }
  return Number(t);
}
