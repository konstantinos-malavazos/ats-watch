...
export function renderDiscord(jobs, ranks = null, variants = null) {
  ...
  const header = ranks
    ? `${count} · ${summariseBands(ordered, ranks)}`
...
/**
 * Summarise the roles by verdict band, e.g. "1 send your CV, 3 worth a look".
 *
 * It reads off BANDS, the same source as the per-role badges, because the two
 * used to disagree: the header counted only 8+ as "worth applying to" while
 * every role below it still carried a yellow "worth a look" badge, so a digest
 * of four 7s announced itself as "0 worth applying to" and then told you to
 * look at all four. Only non-empty bands appear, in descending score order.
 */
function summariseBands(ordered, ranks) {
  const counts = new Map();
  let unranked = 0;
  for (const job of ordered) {
    const score = scoreOf(ranks, job);
    // -1 is "the ranker returned no entry for this job"; it has no badge
    // either, so it must not be counted into a band. It is counted separately
    // and named in the header instead: these roles are in the digest because
    // --min-score cannot hold back a job it has no score for, and a header
    // reading "9 new roles - 1 send your CV" above eight unexplained entries
    // reads as eight recommendations the badges never made.
    if (score < 0) { unranked += 1; continue; }
    const { verdict } = bandFor(score);
    counts.set(verdict, (counts.get(verdict) || 0) + 1);
  }
  const parts = BANDS
    .filter((b) => counts.has(b.verdict))
    .map((b) => `${counts.get(b.verdict)} ${b.verdict}`);
  if (unranked) parts.push(`${unranked} unranked`);
  return parts.length ? parts.join(', ') : 'unscored';
}
