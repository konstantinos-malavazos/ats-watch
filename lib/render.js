 ...
 */
export function renderDiscord(jobs, ranks = null) {
  if (!jobs.length) return '';
  ...
  return [header, ...ordered.map((j) => renderDiscordJob(j, ranks && ranks.get(j.id)))]
    .join('\n\n');
}
...
function renderDiscordJob(job, rank) {
  const lines = [];
  ...
  const meta = [job.company, where, job.salary].filter(Boolean).join(' · ');
